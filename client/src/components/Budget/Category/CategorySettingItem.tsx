...
function CategorySettingItem(props: {
  ...
  const checkedHandler = (event: React.ChangeEvent<HTMLInputElement>) => {
    props.setIsChecked && props.setIsChecked(event.target.value, event.target.checked);
  };
  ...
  return (
    <li
      id={props.id}
      className={`${classes.container} ${props.isDragging ? classes.dragging : ''}`}
    >
      {isCheckItem && (
        <input
      ...
      >
        ...
        <div className={classes.buttons}>
          <Button className={removeClass} styleClass="extra" onClick={removeHandler} />
