...
const WEEK_NAMES: any = {
  ...
  'en-US': ['First week', 'Second week', 'Thired week', 'Fourth week', 'Fifth week'],
};
...
export const getWeekNumbers = (startDate: Date, endDate: Date, locale: string) => {
...
export const getWeekDays = (weekIdx: number, startDate: Date) => {
  ...
  const endOfRequestedWeek = startOfWeek.add(weekIdx + 1, 'week').subtract(1, 'day');
  const dates = [];
  let date = startOfRequestedWeek;

  while (date.isBefore(endOfRequestedWeek) || date.isSame(endOfRequestedWeek, 'day')) {
